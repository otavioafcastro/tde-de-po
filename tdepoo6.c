...
public class PedidoFactory //Eu criei essa classe PedidoFactory só pra deixar a parte de criar pedidos separada da lógica do pedido em si. Achei que assim o código ficaria mais organizado e limpo. Eu vi que isso se chama padrão Factory, mas fiz do meu jeito, uma forma mais simples.
{
...
class Program
{
    static void Main(string[] args)
    {
        ...
        {
            totalSemDesconto += item.Subtotal();
        }

        var desconto = new DescontoFixoPorQuantidade(pedido.Itens);
        double totalComDesconto = pedido.CalcularTotal(desconto);
        double valorDoDesconto = totalSemDesconto - totalComDesconto;
        ...
        foreach (var item in pedido.Itens)
        {
            Console.WriteLine($"- {item.Produto.Nome} | Quantidade: {item.Quantidade} | Preço unitário: R$ {item.Produto.Preco} | Subtotal: R$ {item.Subtotal()}");
        }

        Console.WriteLine($"Total sem desconto: R$ {totalSemDesconto}");
        Console.WriteLine($"Desconto aplicado: R$ {valorDoDesconto}");
